          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider">
                <th className="p-4 font-bold"><Header label="Data" keyName="date" /></th>
                <th className="p-4 font-bold"><Header label="Descrição" keyName="description" /></th>
                <th className="p-4 font-bold"><Header label="Categoria" keyName="category" /></th>
                <th className="p-4 font-bold"><Header label="Conta" keyName="account" /></th>
                <th className="p-4 font-bold text-right"><Header label="Valor" keyName="amount" /></th>
                <th className="p-4 font-bold text-center"><Header label="Status" keyName="status" /></th>
              ...
              {paginatedData.map(t => {
                ...
                return (
                  <tr key={t.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="p-4"><div className="font-bold text-gray-900">{t.description}</div>{t.installment && <span className="text-[10px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded ml-1 font-bold">x{t.installment.current}</span>}</td>
                    <td className="p-4">{t.type === 'TRANSFER' ? <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold"><ArrowRightLeft size={12} /> Transf.</span> : <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-gray-100 text-xs font-bold" style={{ backgroundColor: category ? `${category.color}15` : '#f3f4f6', color: category?.color || '#4b5563' }}>{category?.name || 'Sem Categoria'}</span>}</td>
                    <td className="p-4 text-sm text-gray-500">{account?.name}</td>
                    <td className={`p-4 text-right font-bold ${t.type === 'INCOME' ? 'text-emerald-600' : 'text-gray-900'}`}>{t.type === 'EXPENSE' && '- '}{formatCurrency(t.amount)}</td>
